...
* [Manuel](#manuel)
    ...
    * [Annexes](#annexes)
        * [La méthode ``help``](#la-m%c3%a9thode-help)
...
Ainsi on ne mélange pas les langages et les codes sources ne sont que plus clairs.
...
Pour combiner le fond et la forme en un document finalisé,
...
Ce qui compte c'est que le document soit lisible à la facon d'un simple fichier texte.
...
Ensuite, je vais créer un dossier Templates dans lequel nous trouverons l'ensemble des fichiers de modèle.
...
Si vous voulez pratiquer de votre côté, cliquez ici pour télécharger l'archive.

Dans le fichier Template_V_3_2.zip, vous trouverez le fichier Template.class.php qui est le moteur au complet
ainsi qu'un dossier Help.
Le dossier Help n'est pas le dossier dans lequel vous allez trouver des fichiers de type readme.
Ce sont des fichiers pour la classe permettant d'afficher son propre manuel sur demande.
...
En résumé, il ne s'agit que d'un simple copier-coller. Il n'y à aucune dépendance de fichier, dans quel cas,
j'aurais été contraint de les joindres dans l'archive .zip.
...
Concernant la nomenclature de mes fichiers, j'ai choisis d'un part qu'il contienne l'extension .tpl
indiquant le rôle du fichier, ainsi que l'extension final .html qui permet ainsi aux logiciels d'utiliser
...
En plus de l'extension ".tpl", je vous conseille d'ajouter l'extension prévue à la suite de ".tpl".
...
Disont qu'il faut utiliser les outils à bon escient.


#### Les variables

Dans mon moteur de Template, vous êtes libre de choisir ce qui caractérise une variable
à la seule conditionque le symbole choisi encadre la variable.
...
Remarque 1 : le moteur est sensible à la casse.
C'est-à-dire qu'il distinguera la variable %VARIABLE% de %variable%.

Remarque 2 : il NE FAUT SURTOUT pas utiliser le délimiteur choisi dans le nom de variable.
...
Il existe un autre type de bloc, c'est le bloc conditionnel. A la différence des blocs répétables, celui-ci s'affiche si la, ou, les conditions sont remplies. Il ne se répète pas, mais il peut très bien être inclus dans un bloc normal. Voici comment déclarer un bloc conditionnel :
...
Le moteur gère également cette structure conditionnel.
...
Le plus est que vous pouvez introduire des variables au sein du code PHP écrit.
Celui-ci est d'abord transcrit en tant que modèle avant d'être évalué en tant que code PHP.

Pour que le code PHP au sein du modèle puisse communiquer avec votre script, la classe Template met à disposition
une variable pseudo-superglobale qui est $_PHP.
En effet, celle-ci permet de jongler entre le script et le modèle,
mais ne perdure pas à la manière de $_SESSION et $_SERVER.
...
Ceci aurait pu être fait à l'aide de bloc normaux et de bloc conditionnel,
...
Mais pour les cas alternatif "ELSEIF" et "ELSE", ces instructions doivent être écrite comme dans un <!-- IF (X) AS Y -->
<!-- ELSE -->
...
Mais retenez qu'il ne sera que temporisé et non traité là où se trouve les instructions <!-- BEGIN_DECLARE -->

Remarque 1 : Utiliser l'instruction "BEGIN_DECLARE" dans un bloc sera transparente.

Remarque 2 : Un bloc utilisé par ses balises dédié est également temporisé et peut être invoqué n'importe où.
...
Il ne le traite pas. Cela permet de mieux organiser ses modèles et codes.
...
relativement à sa position dans le document.



#### Les blocs récursif

Si nous prenons le PHP comme exemple, une fonction est récursive lorsqu'elle est amenée à s'appeler elle-même.
...
L'instruction suivante peut s'avérer très utile lorsqu'une partie de la page est commune à l'ensemble des pages du site
tel qu'un header ou un footer de page. C'est l'instruction d'inclusion de modèle.

Voici l'instruction d'inclusion de template.
...
La première phase consiste à initialiser le moteur. La seconde a pour but de le configurer.
La dernière est la phase de génération du document. Ce découpage n'est pas obligatoire,
mais recommandé pour gagner en lisibilité de code.
...
Ces paramètres peuvent être appelés "pseudo-facultatif", car la classe fonctionne sans eux,
...
non-indispensable pour le bon fonctionnement de la classe. Qui dit "non-indispensable",
ne veux pas dire inutile. Voici la liste des paramètres facultatifs :

* Définir le délimiteur de variable. Celui par défaut c'est le "%",
...
J'ai volontairement configuré l'intégralité des paramètres afin de mettre en avant toutes les méthodes.
...
Les méthodes render() et display() renvoient la classe elle-meme ce qui permet d'enchaîner les méthodes de cette façon :
...
La deuxième étape, c'est votre travail : calculé et générer les données à envoyer.
...
j'ai mentionné deux méthodes sur lesquels je ne me suis pas arrêté qui sont "set_utf8_write_treatment"
et "set_utf8_read_treatment".
...
En faite, le fichier ont été enregistré en ISO et cette page affiche du contenu en UTF-8.
Le code du caractère n'est donc pas le même. La page ne sait donc pas afficher le caractère.
Si l'on génère le document et qu'on l'affiche, nous obtenons le même résultat au niveau de l'encodage.

Lorsqu'il s'agit d'un seul fichier qui n'est pas dynamique on peut très simplement changer l'encodage manuellement.
Imaginons que le fichier est généré par un programme tier et qu'il écrit en AINSI ou ISO,
vous ne pouvez plus procéder manuellement. C'est d'autant plus vrai s'il s'agit de nombreux fichiers.
...
Il est possible que l'écriture soit juste, mais que le système qui doit afficher le contenu soit en ISO.
...
fichiers et systèmes d'affichages. Il faudra jongler avec les deux méthodes pour obtenir le résultat souhaité.
Quoiqu'il en soit, notez bien que "set_utf8_write_treatment" et "set_utf8_read_treatment"
acceptent toutes deux ces mêmes valeurs : "none", "encode" et "decode".
...
Je préfère commencer par la méthode "help".
Comme son nom l'indique, elle a pour but de vous aider.
Mais en quoi elle nous aide cette méthode ?
Tout simplement en listant toutes les méthodes publiques existante avec un récapitulatif des paramètres
admis et émis ainsi qu'une description sur le rôle de la méthode consultée.

Ci-dessous, le résultat lorsqu'elle est invoquée sous cette forme :

````php
/** Invocation de la méthode "help" **/
$moteur->help();
````

Important : Cette méthode génère une sortie (code HTML),
...
il s'insérera dans le contenu là où vous avez executé l'invocation en PHP.
